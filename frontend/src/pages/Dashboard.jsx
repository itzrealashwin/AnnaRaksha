...
export default function Dashboard() {
	...
	} = useRiskBatches(warehouseId, { minRisk: 50, limit: 6 });
	const {
		data: alertsData,
		isLoading: alertsLoading,
	} = useAlerts({ warehouseId, status: 'active', limit: 8 });
		...
		return (
            <Card className="border-dashed">
                <CardHeader>
                    ...
                    <CardDescription>
                        Add or pick a warehouse from the left sidebar to view AI-powered dashboard insights.
                    </CardDescription>
                </CardHeader>
            </Card>
        );
                ...
                <Card className="lg:col-span-2 flex flex-col">
                    ...
                    <CardFooter className="flex-col gap-2 text-sm">
                        <div className="flex items-center gap-2 font-medium leading-none">
                            {overview?.currentStock?.toLocaleString()} / {overview?.capacity?.toLocaleString()} units
                        </div>
                        <div className="leading-none text-muted-foreground">
                            Status: <Badge variant="outline" className="capitalize ml-1">{overview?.status || "active"}</Badge>
                        </div>
                    </CardFooter>
                </Card>
                    ...
                    <Card>
                        <CardHeader className="pb-2">
                            ...
                            <CardTitle className="text-4xl text-emerald-600">
                                {overviewLoading ? <Skeleton className="h-9 w-20" /> : (overview?.totalBatches ?? 0)}
                            </CardTitle>
                        </CardHeader>
                        <CardContent>
                        ...
                        <CardFooter className="p-4 pt-0">
                            {/* Micro chart visualization could go here */}
                             ...
                             <span className="text-xs text-emerald-600 font-medium">+2.5% this week</span> {/* Mock metric */}
                        </CardFooter>
                    </Card>
                    ...
                    <Card>
                         <CardHeader className="pb-2">
                            ...
                            <CardTitle className="text-4xl text-amber-600">
                                {overviewLoading ? <Skeleton className="h-9 w-20" /> : (overview?.riskBatchCount ?? 0)}
                            </CardTitle>
                        </CardHeader>
                        <CardContent>
                            <div className="text-xs text-muted-foreground">
                    ...
                    <Card>
                        <CardHeader className="pb-2">
                            ...
                            <CardTitle className="text-4xl text-red-600">
                                {overviewLoading ? <Skeleton className="h-9 w-20" /> : (overview?.criticalRiskCount ?? 0)}
                            </CardTitle>
                        </CardHeader>
                         <CardContent>
                            <div className="text-xs text-muted-foreground">
						...
						<CardFooter className="p-4 pt-0">
							...
							<span className="text-xs text-red-600 font-medium">Critical condition</span>
						</CardFooter>
                    </Card>
							...
							{inventoryLoading ? (
								...
								<ChartContainer
									config={{
										...
										nearExpiry: { label: "Near Expiry", color: "hsl(var(--chart-2))" },
									}}
									className="h-[180px] w-full"
								>
									<BarChart accessibilityLayer data={inventorySeries}>
										...
										<Bar dataKey="nearExpiry" fill="var(--color-nearExpiry)" radius={4} />
									</BarChart>
								</ChartContainer>
							) : (
						...
						{environmentLoading ? (
							...
							<ChartContainer
								config={{
									temperature: {
										label: "Temperature",
										color: "hsl(var(--chart-5))", // Amber/Oracle like
									},
									humidity: {
										label: "Humidity",
										color: "hsl(var(--chart-2))", // Blue/Teal like
									},
								}}
                                    ...
                                    {alerts.map((alert) => (
                                        ...
                                        >
                                            <div className="flex items-center justify-between">
                                                <Badge variant={riskBadgeVariant(alert.riskScore)} className="text-[10px] h-5 px-1.5 uppercase">
                                            ...
                                            <p className="text-sm font-medium leading-none mt-1">
                                                {alert.batchId?.batchId || "Unknown Batch"} - {alert.alertType}
                                            </p>
